...
public class ShotGuide : MonoBehaviour {
	...
	void Update () {
		if (player.playerState != PlayerController.PlayerState.Charging) {
			...
			mousePos = Vector3.ClampMagnitude (new Vector3 (mousePos.x, player.transform.position.y, mousePos.z), length) *3 ;
	...
	void CalculateRicochet(Vector3 mousePos){
		...
		Ray shotRay = new Ray (player.transform.position, mousePos);
		RaycastHit hit;
	...
	bool Deflect(Ray ray, out Ray deflected, out RaycastHit hit){
		int layerMask = 1 << 8;
		layerMask = ~layerMask;
		if (Physics.Raycast (ray, out hit, length * 10000, layerMask)) {
